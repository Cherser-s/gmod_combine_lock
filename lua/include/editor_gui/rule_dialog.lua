...
function PANEL:OnAllowedNext(caller)
	self.NextButton:SetEnabled(true)
end

function PANEL:OnDisallowedNext(caller)
	self.NextButton:SetEnabled(false)
end

function PANEL:GetOnAllowedNext()
	return {
...
function PANEL:SetOnFinishedCallback(delegate)
	if not (isfunction(delegate.OnSuccess) and isfunction(delegate.OnFail)) then
		error("expected function got "..type(delegate))
	end
...
function PANEL:SetCounter(counter)
...
PANEL.RulePicker = vgui.RegisterTable({
	Init = function(self)
...
PANEL.AllowPicker = vgui.RegisterTable({
	Init = function(self)
...
vgui.Register("combine_lock_rule_edit_dialog",PANEL,"DFrame")
COMBINE_LOCK.RuleEditDialog = PANEL
